...
{
        size_t i, msglen;
        pid_t thepid,theppid,theuid,thegid,theeuid,theegid;
        ...
        rcu_read_lock();
        theppid = task_tgid_vnr(rcu_dereference(current->real_parent));
        rcu_read_unlock();

        theuid = from_kuid_munged(current_user_ns(), current_uid());
        thegid = from_kgid_munged(current_user_ns(), current_gid());
        theeuid = from_kuid_munged(current_user_ns(), current_euid());
        theegid = from_kgid_munged(current_user_ns(), current_egid());

        snprintf(message, 200, "Your PID is %d!\nYour PPID is %d!\nYour UID is %d!\nYour GID is %d!\nYour EUID is %d!\nYour EGID is %d!\n", thepid,theppid,theuid,thegid,theeuid,theegid);
